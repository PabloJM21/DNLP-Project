import sys
sys.path.insert(0, "/kaggle/input/dlfnlp/dlfnlp-sose2024")
...
class MultitaskBERT(nn.Module):
    ...
    def predict_similarity(self,
        ...
        # Combine the two embeddings (here, simple concatenation is used; you could also consider other methods like element-wise product or subtraction)
        combined_embedding = torch.cat((embedding_1, embedding_2), dim=1)

        

        # Pass the combined embedding through the similarity prediction layer
        similarity_logit: torch.Tensor = self.similarity_prediction(combined_embedding)
...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=(
            "/kaggle/working/predictions/bert/sst-sentiment-dev-output.csv"
            if not args.task == "multitask"
            else "/kaggle/working/predictions/bert/multitask/sst-sentiment-dev-output.csv"
        ),
    )
    parser.add_argument(
        "--sst_test_out",
        type=str,
        default=(
            "/kaggle/working/predictions/bert/sst-sentiment-test-output.csv"
            if not args.task == "multitask"
            else "/kaggle/working/predictions/bert/multitask/sst-sentiment-test-output.csv"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=(
            "/kaggle/working/predictions/bert/quora-paraphrase-dev-output.csv"
            if not args.task == "multitask"
            else "/kaggle/working/predictions/bert/multitask/quora-paraphrase-dev-output.csv"
        ),
    )
    parser.add_argument(
        "--quora_test_out",
        type=str,
        default=(
            "/kaggle/working/predictions/bert/quora-paraphrase-test-output.csv"
            if not args.task == "multitask"
            else "/kaggle/working/predictions/bert/multitask/quora-paraphrase-test-output.csv"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=(
            "/kaggle/working/predictions/bert/sts-2-similarity-dev-output.csv"
            if not args.task == "multitask"
            else "/kaggle/working/predictions/bert/multitask/sts-similarity-dev-output.csv"
        ),
    )
    parser.add_argument(
        "--sts_test_out",
        type=str,
        default=(
            "/kaggle/working/predictions/bert/sts-2-similarity-test-output.csv"
            if not args.task == "multitask"
            else "/kaggle/working/predictions/bert/multitask/sts-similarity-test-output.csv"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=(
            "/kaggle/working/predictions/bert/etpc-paraphrase-detection-dev-output.csv"
            if not args.task == "multitask"
            else "/kaggle/working/predictions/bert/multitask/etpc-paraphrase-detection-dev-output.csv"
        ),
    )
    parser.add_argument(
        "--etpc_test_out",
        type=str,
        default=(
            "/kaggle/working/predictions/bert/etpc-paraphrase-detection-test-output.csv"
            if not args.task == "multitask"
            else "/kaggle/working/predictions/bert/multitask/etpc-paraphrase-detection-test-output.csv"
        ),
    )
...
if __name__ == "__main__":
    args = get_args()
    args.filepath = f"/kaggle/working/models/{args.option}-{args.epochs}-{args.lr}-{args.task}.pt"  # save path
